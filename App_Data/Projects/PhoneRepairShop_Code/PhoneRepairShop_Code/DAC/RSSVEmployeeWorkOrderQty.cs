...
namespace PhoneRepairShop {
	...
	public class RSSVEmployeeWorkOrderQtyAccumulator : PXAccumulatorAttribute {
		...
		protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns) {
			...
			if (newQty.NbrOfAssignedOrders != null) {
				// Add the restriction for the value of RSSVEmployeeWorkOrderQty.NbrOfAssignedOrders.
				columns.AppendException(
					Messages.ExceedingMaximumNumberOfAssignedWorkOrders,
					new PXAccumulatorRestriction<RSSVEmployeeWorkOrderQty.nbrOfAssignedOrders>(
						PXComp.LE, 10
					)
				);
			}
